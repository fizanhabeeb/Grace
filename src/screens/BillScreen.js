...
export default function BillScreen({ navigation }) {
  ...
  const handleSaveOrder = async () => {
    ...
    if (savedOrder) {
      await clearCurrentOrder(); setCustomerName(''); setTableNumber(''); setPhoneNumber('');
      Alert.alert(t('success'), `${t('orderSaved')} #${billNumber}`, [{ text: 'OK', onPress: () => navigation.navigate('Order') }]);
    } else { Alert.alert(t('error'), 'Failed to save order'); }
  };
  ...
  if (orderItems.length === 0 && !isSaved) {
    return (
      <View style={[globalStyles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <TouchableOpacity style={globalStyles.primaryButton} onPress={() => navigation.navigate('Order')}>
    ...
    <KeyboardAvoidingView style={globalStyles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        <View style={[styles.billHeader, { backgroundColor: theme.primary }]}>
          ...
          <Text style={styles.billInfoText}>{t('billNo')}: #{billNumber}</Text>
        </View>
      ...
      <View style={[styles.actionContainer, { backgroundColor: theme.card, borderTopColor: theme.border, paddingBottom: bottomPadding }]}>
        <TouchableOpacity style={globalStyles.primaryButton} onPress={handleSaveOrder}>
          <Text style={globalStyles.primaryButtonText}>💾 {t('saveOrderHistory')}</Text>
        </TouchableOpacity>
...
// Local styles for things unique to this screen
const styles = StyleSheet.create({
  ...
  billInfoText: { color: '#fff', fontSize: 14 },
